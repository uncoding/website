...
MzTreeView.prototype.nodeInit = function(sourceIndex, parentId)
{
  ...
  var ctrlName = this.getAttribute(source, "ctrlName");        
  var ctrlType = this.getAttribute(source, "ctrlType"); 
...
MzTreeView.prototype.nodeToHTML = function(node, AtEnd)
{
  ...
  if(node.ctrl) HTML += "<input type='"+ node.ctrlType +"' name='"+ node.ctrlName +"' id='"+ this.name +"_checbox_"+ id +"' value='"+ tmpvalue[1] +"' onclick=\""+ this.name +".checkChild('"+ id +"')\" "+ HTMCHKED +">" 
  HTML += "<A "+
    ...
    "onclick=\"return "+ this.name +".nodeClick('"+ id +"', '" + data +"')\">"+ node.text +
    "</A>";
  HTML +=  "</NOBR></DIV>";
...
{
	if(this.node[id].ctrlType=="checkbox")
	{
		this.selChild(id);
		this.selParent(id);
	}
...
//节点链接单击事件处理方法
//id 客户端树节点的 id
MzTreeView.prototype.nodeClick = function(id, node)
{
  var source = this.nodes[this.node[id].sourceIndex];
  eval(this.getAttribute(source, "method"));

  var data = this.getAttribute(source, "data");
  var txtdis = this.getAttribute(source, "text");
  var ParentSource = this.nodes[this.node[this.node[id].parentId].sourceIndex];
  var ParentData = this.getAttribute(ParentSource, "data");
  var txtdis2 = this.getAttribute(ParentSource, "text");
  var ParentID = "";
  if(ParentSource == null) ParentID = "0"; else ParentID = ParentData.split("=")[1];
  this.SetModifyInfo(data.split("=")[1], ParentID, txtdis);
  return !(!this.getAttribute(source, "url") && this.url=="#");
};

//如果页面需要保存节点以便程序修改，那么点击后自动设置内动
MzTreeView.prototype.SetModifyInfo = function(dbID,dbParentID,NodeText)
{
	var objCurrentID = document.getElementById("CurrentID");
	var objParentID = document.getElementById("ParentID");
	var objName = document.getElementById("ModifyName");
	if(objCurrentID != null)
	{
		objCurrentID.value = dbID;
		var objBigDiv = document.getElementById("divBig");
		var objLitDiv = document.getElementById("divLit");
		if(objBigDiv!=null && objLitDiv!=null)
		{
			if(parseInt(dbID)<100000)
			{
				objBigDiv.style.display="block";
				objLitDiv.style.display="none";
			}
			else
			{
				objBigDiv.style.display="none";
				objLitDiv.style.display="block";
			}
		}
		
	}
	if(objParentID != null) objParentID.value = dbParentID;
	if(objName != null) objName.value = NodeText;
