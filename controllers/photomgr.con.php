<?PHP
if (!defined('CLASS_PHOTOMGR'))
{
     define('CLASS_PHOTOMGR','1');
     class photomgr extends manage{
          public function __construct()
          {
               parent::__construct();
  require_once (VENDORS_PATH.'Tree.class.php');
$sql1="SELECT * FROM `cms_classify` where mgr_table='cms_photo' and mgr_status=1 order by mgr_sort asc";
$resultSet3=$this->mydb->execsql($sql1);
$this->cout['photo_category'] = array();
$this->cout['photo_category'][0] = '请选择';
$cate = array();
...
foreach($ordertree as $k => $v){
  ...
  if ($v['lever'] > 1){
    ...
    $this->cout['photo_category'][$v['id']] = $procate.$v['name'];
  }else{
    $this->cout['photo_category'][$v['id']] = $v['name'];
  }
}
            }
          public function photo(){
           ...
           $sql1="SELECT * FROM `cms_photo` ".$where.$orderby."  limit {$row_skip} ,{$row_limit}";
           //T($sql1);
           $resultSet3=$this->mydb->execsql($sql1);
           $sql2="SELECT * FROM `cms_photo` ".$where ;
           ...
           $this->display('photomgr.list.html');
          }
          public function photoadd($oid=0,$id=0){
               if(isPost()){
...
*/
          if ($acttype==1){
               $result=$this->mydb->update('cms_photo','id',$_REQUEST['id']);
          }else{
               $result=$this->mydb->insert('cms_photo');
/*
              $key = str_ireplace('cms_','','photo');
              ...
              $this->mydb->setData('mgr_only', 'cms_photo_'.$result);
...
*/
       ...
       if (@$_REQUEST['acttype'] == 1|| @$_REQUEST['acttype'] == 2 || !empty($id)){
          ...
          $resultSet=$this->mydb->fetch('cms_photo');
       ...
       if (@$_REQUEST['acttype'] == 2){
          $this->display('photomgr.info.html');
       }else{
          $this->display('photomgr.add.html');
       }
    }
    public function photodel(){
     ...
     if(!empty($_REQUEST['id'])){
          $sql="DELETE FROM `cms_photo` WHERE `id` = '".$_REQUEST['id']."' limit 1;";
