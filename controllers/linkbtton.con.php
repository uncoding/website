<?PHP
if (!defined('CLASS_LINKBTTON'))
{
  define('CLASS_LINKBTTON','1');
  class linkbtton extends home
  {
  ...
  if(!empty($_REQUEST['cid']) && is_integer((int)($_REQUEST['cid']))){
    ...
    if (!empty($resultSet3[0])) $this->cout['linkbtton_catatitle']=$resultSet3[0]['shw_title'];
  }
   ...
   foreach ($resultSet3 as $key => $value) {
    ...
    $this->cout['vlist'][] = $value;
        ...
        $this->display('linkbtton.list.html');   
      //}
    }
...
//$this->cout['vinfo']['shw_content'] = $this->keywordurl($this->cout['vinfo']['shw_content']);
    ...
    foreach($ordertree as $k => $v){
      $p[$v['lever']] = '<a href="/index.php?act=linkbtton.linkbtton&cid='.$v['id'].'">'.$v['name'].'</a>'.$v['prestr'];
      if ($v['lever'] > 1){
        $procate = '';
        for($i=1;$i<$v['lever'];$i++){
          $procate = $procate.$p[$i];
        }
        $this->cout['linkbtton_category'][$v['id']] = $procate.'<a href="/index.php?act=linkbtton.linkbtton&cid='.$v['id'].'">'.$v['name'].'</a>';
      }else{
        $this->cout['linkbtton_category'][$v['id']] = '<a href="/index.php?act=linkbtton.linkbtton&cid='.$v['id'].'">'.$v['name'].'</a>';
      }
    }
           $this->display('linkbtton.info.html');
        }   
