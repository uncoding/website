<?PHP
if (!defined('CLASS_COMMENTMGR'))
{
     define('CLASS_COMMENTMGR','1');
     class commentmgr extends manage{
          public function __construct()
          {
               parent::__construct();
  require_once (VENDORS_PATH.'Tree.class.php');
$sql1="SELECT * FROM `cms_classify` where mgr_table='cms_comment' and mgr_status=1 order by mgr_sort asc";
$resultSet3=$this->mydb->execsql($sql1);
$this->cout['comment_category'] = array();
$this->cout['comment_category'][0] = '请选择';
$cate = array();
...
foreach($ordertree as $k => $v){
  ...
  if ($v['lever'] > 1){
    ...
    $this->cout['comment_category'][$v['id']] = $procate.$v['name'];
  }else{
    $this->cout['comment_category'][$v['id']] = $v['name'];
  }
}
            }
          public function comment(){
           ...
           $sql1="SELECT * FROM `cms_comment` ".$where.$orderby."  limit {$row_skip} ,{$row_limit}";
           //T($sql1);
           $resultSet3=$this->mydb->execsql($sql1);
           $sql2="SELECT * FROM `cms_comment` ".$where ;
           ...
           $this->display('commentmgr.list.html');
          }
          public function commentadd($oid=0,$id=0){
               if(isPost()){
...
fclose($fp2);
  ...
  $_REQUEST['item_shw_content'] = str_replace($srcfile, $urlfile, $_REQUEST['item_shw_content']);
              ...
              $acttype=$_REQUEST['acttype'];
                            $this->mydb->setData('shw_cid', $_REQUEST['item_shw_cid']);
                            $this->mydb->setData('shw_lid', $_REQUEST['item_shw_lid']);
            ...
            $sensitivestr = "";
if (!empty($_REQUEST['item_shw_cid'])){
$sensitivestr .= $_REQUEST['item_shw_cid'].'|';
}
if (!empty($_REQUEST['item_shw_lid'])){
$sensitivestr .= $_REQUEST['item_shw_lid'].'|';
}
...
if (!empty($word)){
  ...
  die($obj);
          ...
          if ($acttype==1){
               $result=$this->mydb->update('cms_comment','id',$_REQUEST['id']);
          }else{
               $result=$this->mydb->insert('cms_comment');
              $key = str_ireplace('cms_','','comment');
              ...
              $this->mydb->setData('mgr_only', 'cms_comment_'.$result);
       ...
       if (@$_REQUEST['acttype'] == 1|| @$_REQUEST['acttype'] == 2 || !empty($id)){
          ...
          $resultSet=$this->mydb->fetch('cms_comment');
       ...
       if (@$_REQUEST['acttype'] == 2){
          $this->display('commentmgr.info.html');
       }else{
          $this->display('commentmgr.add.html');
       }
    }
    public function commentdel(){
     ...
     if(!empty($_REQUEST['id'])){
          $sql="DELETE FROM `cms_comment` WHERE `id` = '".$_REQUEST['id']."' limit 1;";
