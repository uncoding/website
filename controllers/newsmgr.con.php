<?PHP
if (!defined('CLASS_NEWSMGR'))
{
     define('CLASS_NEWSMGR','1');
     class newsmgr extends manage{
...
foreach($ordertree as $k => $v){
  ...
  if ($v['lever'] > 1){
    ...
    $this->cout['news_category'][$v['id']] = $procate.$v['name'];
  }else{
    $this->cout['news_category'][$v['id']] = $v['name'];
  }
}
            }
          public function news(){
           ...
           $this->display('newsmgr.list.html');
          }
          public function newsadd($oid=0,$id=0){
               if(isPost()){
...
*/
          if ($acttype==1){
               $result=$this->mydb->update('cms_news','id',$_REQUEST['id']);
          }else{
               $result=$this->mydb->insert('cms_news');
	/*
              $key = str_ireplace('cms_','','news');
              ...
              $this->mydb->setData('mgr_only', 'cms_news_'.$result);
	...
	*/
       ...
       if (@$_REQUEST['acttype'] == 2){
          $this->display('newsmgr.info.html');
       }else{
          $this->display('newsmgr.add.html');
       }
    }
    public function newsdel(){
     ...
     if(!empty($_REQUEST['id'])){
          $sql="DELETE FROM `cms_news` WHERE `id` = '".$_REQUEST['id']."' limit 1;";
