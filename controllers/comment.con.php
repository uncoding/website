<?PHP
if (!defined('CLASS_COMMENT'))
{
  define('CLASS_COMMENT','1');
  ...
  if(!empty($_REQUEST['cid']) && is_integer((int)($_REQUEST['cid']))){
    ...
    if (!empty($resultSet3[0])) $this->cout['comment_catatitle']=$resultSet3[0]['shw_title'];
  }
...
//$this->cout['vinfo']['shw_content'] = $this->keywordurl($this->cout['vinfo']['shw_content']);
    ...
    foreach($ordertree as $k => $v){
      $p[$v['lever']] = '<a href="/index.php?act=comment.comment&cid='.$v['id'].'">'.$v['name'].'</a>'.$v['prestr'];
      if ($v['lever'] > 1){
        $procate = '';
        for($i=1;$i<$v['lever'];$i++){
          $procate = $procate.$p[$i];
        }
        $this->cout['comment_category'][$v['id']] = $procate.'<a href="/index.php?act=comment.comment&cid='.$v['id'].'">'.$v['name'].'</a>';
      }else{
        $this->cout['comment_category'][$v['id']] = '<a href="/index.php?act=comment.comment&cid='.$v['id'].'">'.$v['name'].'</a>';
      }
    }
           $this->display('comment.info.html');
        }   

    public function addInfo(){
        $this->mydb->setData('shw_cid', $_REQUEST['ctitle']);
        $this->mydb->setData('shw_lid', $_REQUEST['ltitle']);
        $this->mydb->setData('shw_title', $_REQUEST['gname']);
        $this->mydb->setData('shw_content', $_REQUEST['content']);
        $this->mydb->setData('mgr_status', 0);
        $this->mydb->setData('log_user','admin');
