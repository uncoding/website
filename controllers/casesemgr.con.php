<?PHP
if (!defined('CLASS_CASESEMGR'))
{
     define('CLASS_CASESEMGR','1');
     class casesemgr extends manage{
...
$this->cout['casese_category'][0] = '请选择';
$cate = array();
...
foreach($ordertree as $k => $v){
  ...
  if ($v['lever'] > 1){
    ...
    $this->cout['casese_category'][$v['id']] = $procate.$v['name'];
  }else{
    $this->cout['casese_category'][$v['id']] = $v['name'];
  }
}
            }
          public function casese(){
           ...
           $this->display('casesemgr.list.html');
          }
          public function caseseadd($oid=0,$id=0){
               if(isPost()){
...
*/
          if ($acttype==1){
               $result=$this->mydb->update('cms_casese','id',$_REQUEST['id']);
          }else{
               $result=$this->mydb->insert('cms_casese');
/*
              $key = str_ireplace('cms_','','casese');
              ...
              $this->mydb->setData('mgr_only', 'cms_casese_'.$result);
...
*/
       ...
       if (@$_REQUEST['acttype'] == 2){
          $this->display('casesemgr.info.html');
       }else{
          $this->display('casesemgr.add.html');
       }
    }
    public function casesedel(){
     ...
     if(!empty($_REQUEST['id'])){
          $sql="DELETE FROM `cms_casese` WHERE `id` = '".$_REQUEST['id']."' limit 1;";
