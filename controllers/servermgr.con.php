<?PHP
if (!defined('CLASS_SERVERMGR'))
{
     define('CLASS_SERVERMGR','1');
     class servermgr extends manage{
...
$this->cout['server_category'][0] = '请选择';
$cate = array();
...
foreach($ordertree as $k => $v){
  ...
  if ($v['lever'] > 1){
    ...
    $this->cout['server_category'][$v['id']] = $procate.$v['name'];
  }else{
    $this->cout['server_category'][$v['id']] = $v['name'];
  }
}
            }
          public function server(){
           ...
           $this->display('servermgr.list.html');
          }
          public function serveradd($oid=0,$id=0){
               if(isPost()){
...
*/
          if ($acttype==1){
               $result=$this->mydb->update('cms_server','id',$_REQUEST['id']);
          }else{
               $result=$this->mydb->insert('cms_server');
/*
              $key = str_ireplace('cms_','','server');
              ...
              $this->mydb->setData('mgr_only', 'cms_server_'.$result);
...
*/
       ...
       if (@$_REQUEST['acttype'] == 2){
          $this->display('servermgr.info.html');
       }else{
          $this->display('servermgr.add.html');
       }
    }
    public function serverdel(){
     ...
     if(!empty($_REQUEST['id'])){
          $sql="DELETE FROM `cms_server` WHERE `id` = '".$_REQUEST['id']."' limit 1;";
