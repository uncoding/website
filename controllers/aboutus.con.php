<?PHP
if (!defined('CLASS_ABOUTUS'))
{
  define('CLASS_ABOUTUS','1');
  class aboutus extends home
  {
    public function __construct()
    {
      ...
      $abtCls=$this->mydb->execsql("SELECT * FROM `cms_classify` where mgr_status=1 and mgr_table='cms_aboutus' order by mgr_sort asc");
      $this->cout['abtCls']= $abtCls;
    }
  ...
  if(!empty($_REQUEST['cid']) && is_integer((int)($_REQUEST['cid']))){
    ...
    if (!empty($resultSet3[0])) $this->cout['aboutus_catatitle']=$resultSet3[0]['shw_title'];
  }
      ...
      $sql1="SELECT * FROM `cms_aboutus` ".$where.$orderby."  limit {$row_skip} ,{$row_limit}";
      //T($sql1);
      $resultSet3=$this->mydb->execsql($sql1);
      $sql2="SELECT * FROM `cms_aboutus` ".$where ;
   ...
   foreach ($resultSet3 as $key => $value) {
    ...
    $this->cout['vlist'][] = $value;
        ...
        $this->display('aboutus.list.html');   
      //}
    }
    public function info($id=0){
           ...
           if (!empty($id) && is_integer((int)($id))){
                ...
                $resultSet=$this->mydb->fetch('cms_aboutus');
...
//$this->cout['vinfo']['shw_content'] = $this->keywordurl($this->cout['vinfo']['shw_content']);
    ...
    $sql1="SELECT * FROM `cms_classify` where mgr_table='cms_aboutus' and mgr_status=1 order by mgr_sort asc";
    $resultSet3=$this->mydb->execsql($sql1);
    $this->cout['aboutus_category'] = array();
    $cate = array();
    foreach ($resultSet3 as $key => $value) {
    ...
    foreach($ordertree as $k => $v){
      $p[$v['lever']] = '<a href="/index.php?act=aboutus.aboutus&cid='.$v['id'].'">'.$v['name'].'</a>'.$v['prestr'];
      if ($v['lever'] > 1){
        $procate = '';
        for($i=1;$i<$v['lever'];$i++){
          $procate = $procate.$p[$i];
        }
        $this->cout['aboutus_category'][$v['id']] = $procate.'<a href="/index.php?act=aboutus.aboutus&cid='.$v['id'].'">'.$v['name'].'</a>';
      }else{
        $this->cout['aboutus_category'][$v['id']] = '<a href="/index.php?act=aboutus.aboutus&cid='.$v['id'].'">'.$v['name'].'</a>';
      }
    }
           $this->display('aboutus.info.html');
        }   
