<?PHP
if (!defined('CLASS_CASESE'))
{
  define('CLASS_CASESE','1');
  class casese extends home
  {
  ...
  if(!empty($_REQUEST['cid']) && is_integer((int)($_REQUEST['cid']))){
    ...
    if (!empty($resultSet3[0])) $this->cout['casese_catatitle']=$resultSet3[0]['shw_title'];
  }
      ...
      $sql1="SELECT * FROM `cms_casese` ".$where.$orderby."  limit {$row_skip} ,{$row_limit}";
      //T($sql1);
      $resultSet3=$this->mydb->execsql($sql1);
      $sql2="SELECT * FROM `cms_casese` ".$where ;
   ...
   foreach ($resultSet3 as $key => $value) {
    ...
    public function info($id=0){
           ...
           if (!empty($id) && is_integer((int)($id))){
                ...
                $resultSet=$this->mydb->fetch('cms_casese');
...
//$this->cout['vinfo']['shw_content'] = $this->keywordurl($this->cout['vinfo']['shw_content']);
            ...
            @$id = $_REQUEST['id']; 
           $cid=$this->mydb->execsql("SELECT * FROM `cms_casese` WHERE id = {$id} limit 1");
           $cname = $this->mydb->execsql("SELECT * FROM `cms_classify` WHERE id = {$cid[0]['cid']} limit 1");
           $this->cout['casese_catatitle']=$cname[0]['shw_title'];
            $this->cout['cid']=$cid[0]['id'];
           // ## 下一页
           $sql1="SELECT * FROM `cms_casese` WHERE id<{$id} and cid={$cid[0]['cid']} ORDER BY log_time desc LIMIT 1 ";
            $resultSet3=$this->mydb->execsql($sql1);
    ...
    $sql1="SELECT * FROM `cms_classify` where mgr_table='cms_casese' and mgr_status=1 order by mgr_sort asc";
    $resultSet3=$this->mydb->execsql($sql1);
    $this->cout['casese_category'] = array();
    $cate = array();
    foreach ($resultSet3 as $key => $value) {
    ...
    foreach($ordertree as $k => $v){
      $p[$v['lever']] = '<a href="/index.php?act=casese.casese&cid='.$v['id'].'">'.$v['name'].'</a>'.$v['prestr'];
      if ($v['lever'] > 1){
        $procate = '';
        for($i=1;$i<$v['lever'];$i++){
          $procate = $procate.$p[$i];
        }
        $this->cout['casese_category'][$v['id']] = $procate.'<a href="/index.php?act=casese.casese&cid='.$v['id'].'">'.$v['name'].'</a>';
      }else{
        $this->cout['casese_category'][$v['id']] = '<a href="/index.php?act=casese.casese&cid='.$v['id'].'">'.$v['name'].'</a>';
      }
    }
           $this->display('casese.info.html');
        }   
