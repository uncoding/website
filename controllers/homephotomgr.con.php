<?PHP
if (!defined('CLASS_HOMEPHOTOMGR'))
{
     define('CLASS_HOMEPHOTOMGR','1');
     class homephotomgr extends manage{
...
$this->cout['homephoto_category'][0] = '请选择';
$cate = array();
...
foreach($ordertree as $k => $v){
  ...
  if ($v['lever'] > 1){
    ...
    $this->cout['homephoto_category'][$v['id']] = $procate.$v['name'];
  }else{
    $this->cout['homephoto_category'][$v['id']] = $v['name'];
  }
}
            }
          public function homephoto(){
           ...
           $this->display('homephotomgr.list.html');
          }
          public function homephotoadd($oid=0,$id=0){
               if(isPost()){
...
*/
          if ($acttype==1){
               $result=$this->mydb->update('cms_homephoto','id',$_REQUEST['id']);
          }else{
               $result=$this->mydb->insert('cms_homephoto');
/*
              $key = str_ireplace('cms_','','homephoto');
              ...
              $this->mydb->setData('mgr_only', 'cms_homephoto_'.$result);
...
*/
          ...
          if(!empty($result)){
               $obj='{
                          ...
                          "navTabId":"homephotomgr.homephoto", 
                          "rel":"", 
       ...
       if (@$_REQUEST['acttype'] == 2){
          $this->display('homephotomgr.info.html');
       }else{
          $this->display('homephotomgr.add.html');
       }
    }
    public function homephotodel(){
     ...
     if(!empty($_REQUEST['id'])){
          $sql="DELETE FROM `cms_homephoto` WHERE `id` = '".$_REQUEST['id']."' limit 1;";
