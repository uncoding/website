<?PHP
if (!defined('CLASS_PHOTO'))
{
  define('CLASS_PHOTO','1');
...
//$this->cout['vinfo']['shw_content'] = $this->keywordurl($this->cout['vinfo']['shw_content']);
    ...
    foreach($ordertree as $k => $v){
      $p[$v['lever']] = '<a href="/index.php?act=photo.photo&cid='.$v['id'].'">'.$v['name'].'</a>'.$v['prestr'];
      if ($v['lever'] > 1){
        $procate = '';
        for($i=1;$i<$v['lever'];$i++){
          $procate = $procate.$p[$i];
        }
        $this->cout['photo_category'][$v['id']] = $procate.'<a href="/index.php?act=photo.photo&cid='.$v['id'].'">'.$v['name'].'</a>';
      }else{
        $this->cout['photo_category'][$v['id']] = '<a href="/index.php?act=photo.photo&cid='.$v['id'].'">'.$v['name'].'</a>';
      }
    }
           $this->display('photo.info.html');
        }   
