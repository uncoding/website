<?PHP
if (!defined('CLASS_ABOUTUSMGR'))
{
     define('CLASS_ABOUTUSMGR','1');
     class aboutusmgr extends manage{
...
$this->cout['aboutus_category'][0] = '请选择';
$cate = array();
...
foreach($ordertree as $k => $v){
  ...
  if ($v['lever'] > 1){
    ...
    $this->cout['aboutus_category'][$v['id']] = $procate.$v['name'];
  }else{
    $this->cout['aboutus_category'][$v['id']] = $v['name'];
  }
}
            }
          public function aboutus(){
           ...
           $this->display('aboutusmgr.list.html');
          }
          public function aboutusadd($oid=0,$id=0){
               if(isPost()){
...
*/
          if ($acttype==1){
               $result=$this->mydb->update('cms_aboutus','id',$_REQUEST['id']);
          }else{
               $result=$this->mydb->insert('cms_aboutus');
		/*
              $key = str_ireplace('cms_','','aboutus');
              ...
              $this->mydb->setData('mgr_only', 'cms_aboutus_'.$result);
       ...
       if (@$_REQUEST['acttype'] == 2){
          $this->display('aboutusmgr.info.html');
       }else{
          $this->display('aboutusmgr.add.html');
       }
    }
    public function aboutusdel(){
     ...
     if(!empty($_REQUEST['id'])){
          $sql="DELETE FROM `cms_aboutus` WHERE `id` = '".$_REQUEST['id']."' limit 1;";
